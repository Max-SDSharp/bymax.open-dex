  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Bymax OpenDEX [Under construction]</h1>
        <ThemeToggle />
      </div>
      
      {/* Wallet Connection */}
      <div className="mb-8 p-4 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 shadow-sm">
        <h2 className="text-xl font-semibold mb-4 text-gray-800 dark:text-white">Wallet</h2>
        {wallet.connected ? (
          <div>
            <p className="text-gray-700 dark:text-gray-300">Connected: <span className="font-mono">{wallet.address}</span></p>
            <h3 className="mt-2 font-medium text-gray-800 dark:text-white">Balances:</h3>
      ...
      {/* Market Selection */}
      <div className="mb-8 p-4 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 shadow-sm">
        <h2 className="text-xl font-semibold mb-4 text-gray-800 dark:text-white">Markets</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {Object.entries(markets).map(([marketName, marketData]) => (
            <div 
              key={marketName}
              onClick={() => selectMarket(marketName)}
            ...
            >
              <h3 className="font-medium text-gray-800 dark:text-white">{marketName}</h3>
              <p className="text-xl font-bold text-gray-900 dark:text-white">${marketData.price.toLocaleString()}</p>
              <p className={`text-sm ${marketData.change24h >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                {marketData.change24h >= 0 ? '+' : ''}{marketData.change24h}%
              </p>
              <p className="text-xs text-gray-500 dark:text-gray-400">Vol: ${(marketData.volume24h / 1000000).toFixed(1)}M</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {wallet.connected && selectedMarket && (
        <div className="mb-8 p-4 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 shadow-sm">
          <h2 className="text-xl font-semibold mb-4 text-gray-800 dark:text-white">Trade {selectedMarket}</h2>
      ...
      {wallet.connected && (
        <div className="p-4 border dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 shadow-sm">
          <h2 className="text-xl font-semibold mb-4 text-gray-800 dark:text-white">Orders</h2>
          
          {orders.pending.length > 0 && (
            <>
              <h3 className="font-medium mb-2 text-gray-800 dark:text-white">Pending Orders</h3>
              <div className="overflow-x-auto mb-6">
                <table className="min-w-full">
                  <thead>
                    <tr className="border-b dark:border-gray-700">
                      <th className="py-2 text-left text-gray-700 dark:text-gray-300">Market</th>
                      <th className="py-2 text-left text-gray-700 dark:text-gray-300">Type</th>
                      <th className="py-2 text-left text-gray-700 dark:text-gray-300">Side</th>
                      <th className="py-2 text-left text-gray-700 dark:text-gray-300">Amount</th>
                      <th className="py-2 text-left text-gray-700 dark:text-gray-300">Price</th>
                    </tr>
                  </thead>
                  <tbody>
                    {orders.pending.map(order => (
                      <tr key={order.id} className="border-b dark:border-gray-700">
                        <td className="py-2 text-gray-700 dark:text-gray-300">{order.market}</td>
                        <td className="py-2 text-gray-700 dark:text-gray-300">{order.type}</td>
                        <td className="py-2 font-medium" style={{ color: order.side === 'buy' ? '#10b981' : '#ef4444' }}>
                          {order.side}
                        </td>
                        <td className="py-2 text-gray-700 dark:text-gray-300">{order.amount}</td>
                        <td className="py-2 text-gray-700 dark:text-gray-300">{order.price || 'Market'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {orders.completed.length > 0 && (
            <>
              <h3 className="font-medium mb-2 text-gray-800 dark:text-white">Completed Orders</h3>
